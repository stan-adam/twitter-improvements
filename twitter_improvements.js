(async function() {
    ...
    const username_queryselector = 'div[data-testid="User-Name"]';
    ...
    function getLinkFunctions() {
        class Links {
            ...
            static async getSplitURLFromTweet(tweet) {
                let name = tweet.querySelector(username_queryselector).querySelector('a').href;
                let url = Array.from(tweet.querySelectorAll('a'))
                            .map(link => link.href)
                            .filter(link => link.includes(name))
                            .map(link => link.split("/"))
